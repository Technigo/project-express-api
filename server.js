...
app.get('/books', (req, res) => {
  ...
  if (title) {
    booksList = booksList.filter(book => (
      book.title.toString().includes(title)
      || book.title.toString().toLowerCase().includes(title)
    ))
  }
  // PAGINATION (Query by page and/or page-size):
  ...
  const bookPage = booksList.slice(startIndex, endIndex) //Creating pages using slice.
  const returnObject = {
...
app.get('/books/isbn/:isbn', (req, res) => {
  ...
  const singleBookIsbn = booksData.find((book) => (
...
app.get('/books/author/:author', (req, res) => {
  ...
  if (author) {
    const booksFilteredByAuthor = booksData.filter(book => (
      book.authors.includes(author) || book.authors.toLowerCase().includes(author)
    ))
